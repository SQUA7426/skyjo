...
case class DiscardPile(
  ...
  override def getDiscCard(ctrl: ControllerInterface): Option[CardInterface] =
    if discPile == "Disc" || discPile == "" then None
    else Some(ctrl.toCard(discPile).trueCopy)

  def isTurned: Boolean = turned

  override def putToDiscardPile(
      from: Any,
    ...
    from match {
      ...
      case _ =>
        throw new MatchError(s"Connot process this type: ${from.getClass}")
    }
  }
