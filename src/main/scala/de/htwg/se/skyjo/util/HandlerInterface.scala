...
// }
//
// class UndoHandler(ctrl: ControllerInterface, override val next: Option[Handler])
//     extends Handler {
...
// }
//
// class FlipHandler(ctrl: ControllerInterface, override val next: Option[Handler])
//     extends Handler {
//   override def handle(request: String, state: GameState): Option[GameState] =
...
// }
//
// class DiscardActionHandler(
//     ctrl: ControllerInterface,
//     override val next: Option[Handler]
// ) extends Handler:
//   override def handle(request: String, state: GameState): Option[GameState] = {
//     // Wenn "2" gedrückt wird und eine Karte gehalten wird
//     if (request == "s" && state.drawnCard.isDefined) {
//       ctrl.discardDrawnCard()
//       Some(ctrl.getGameState)
//     } else {
//       next.flatMap(_.handle(request, state))
//     }
//   }
//
// class SwapHandler(ctrl: ControllerInterface, override val next: Option[Handler])
//     extends Handler {
//   override def handle(request: String, state: GameState): Option[GameState] =
...
// }
//
// class DiscHandler(ctrl: ControllerInterface, override val next: Option[Handler])
//     extends Handler:
//
//   override def handle(request: String, state: GameState): Option[GameState] =
//     if (request == "0" && state.drawnCard.isEmpty) {
//
...
//     } else next.flatMap(_.handle(request, state))
//
// class DeckHandler(ctrl: ControllerInterface, override val next: Option[Handler])
//     extends Handler:
//   override def handle(request: String, state: GameState): Option[GameState] =
//     // Nur reagieren, wenn man noch keine Karte hat und nicht flippen muss
...
//   private val chain = new UndoHandler(
//     ctrl,
//     Some(
//       new FlipHandler(
//         ctrl,
//         Some(
//           new DiscardActionHandler(
//             ctrl,
//             Some(
//               new SwapHandler(
//                 ctrl,
...
//                   new DiscHandler(
//                     ctrl,
//                     Some(new DeckHandler(ctrl, Some(new LastHandler())))
//                   )
//                 )
//               )
//             )
